...
def transform_flow(df, gas_date):
    df = df[['Gas Day','Hour','MCF']]
    df.rename(columns={'Hour':'Hr','MCF':'Consumption'})
    df['Meter'] = "meter"
    print(df)
...
def get_table_details(web_api_url,access_token, table_name, id_col="", name_col="", url="" ):
    ...
    try:
        ...
        # Handle HTTP errors (non-200 status codes)
        logging.info(f"HTTP Error: {e.response.status_code} - {e.response.reason}")
        raise
    except requests.exceptions.RequestException as e:
        # Handle connection errors, timeouts, and general request exceptions
...
def get_flow_values(web_api_url, access_token, gas_date, hour, entity):
    ...
    try:
        ...
        mcf_list=[]
        response_dict = get_table_details(web_api_url, access_token, table_name, url=url)
        if response_dict is not None:
            ...
            if status=="success" and len(resp_list)>0:
                ...
                logging.warn("sfdadsfa")
                logging.warn(mcf_df)
                return "success",mcf_df
        else:
            logger.error(f"Error in getting existing MCF records for {entity}: {e}")
            return "fail",None

    except Exception as e:
        logger.error(f"Error in getting existing MCF records for {entity}: {e}")
        mcf_df = pd.concat([mcf_df, pd.DataFrame([{'Gas Day':gas_date, 'MCF':None }])], ignore_index=True)
        return "fail",None

        # raise

def get_raw_data(web_api_url, access_token, gas_date, hour):
    perryK_df = get_flow_values(web_api_url, access_token, gas_date, hour, PERRYK_ENTITY)
    return perryK_df
...
#11-24-1-10
    ...
    job_start_time_est = datetime.now(eastern) -  timedelta(days=6)
    ...
    logger.info(f"Getting data from dataverse  {gas_date} {hour}")
    print(f"Getting data from dataverse  {gas_date} {hour}")
    status, source_df = get_raw_data(web_api_url, access_token, gas_date,hour)
    logger.info(f"Transforming data {source_df}")
    ...
    write_to_network_dir(web_api_url, access_token, env_url, gas_date, processed_df)    
    ...
    try:
        ...
        logger.error(f"UPLOAD FOR {gas_date} UNSUCCESSFULL {e}")
        print(f"UPLOAD FOR {gas_date} UNSUCCESSFULL {e}")
...
if __name__=="__main__":
    ...
    parser.add_argument('--gas_date', type=str, help='The date you want to upload in yyyy-mm-dd format; If left empty, program will import today\'s perryK data',default='auto')
    parser.add_argument('--hour', type=str, help='The hour you want to upload as a number; If left empty, program will import all of today\'s perryK data',default='auto')
    parser.add_argument('--env', choices=['dev', 'stage'], help='Environment', default='dev')
    args = parser.parse_args()
    perryK_upload(args.gas_date, args.hour,args.env)
